...
void uci_element_check_option(struct uci_option *option, char* type, char** value)
{
...
int uci_parseServerName(struct uci_package *package, char **ServerName)
{
	...
	uci_foreach_element(&package->sections, i){
		...
		if(strcmp(section_type,"openvpn") == 0){
			...
			uci_element_check_option(option,"_name", ServerName);
			uci_element_check_option(option,"manport", &port);
		}
